...
uvec3 read_face_index(
  uint mesh_id,
  uint primitive_id
) {
  const mesh_type mesh = mesh_pool[ mesh_id ];

  const uint global_vertex_id = primitive_id * 3u;
...
void read_face_position(
  meshlet_reader reader,
  uint face_id,
#ifdef __cplusplus
  face_attribute &f
#else
  inout face_attribute f
#endif
) {
  f.valid = false;
  ...
  uvec3 vertex_index;
  if( accessor_pool[ mesh.accessor + 1 ].type == GCT_SHADER_TYPE_ID_DGF ) {
    if( reader.dgf_info.header.numTriangles <= face_id ) return;
    uvec3 index;
    if( reader.wave_mode ) {
    ...
    if( face_id >= reader.face_count ) return;
...
face_attribute read_face_position2(
  meshlet_reader reader,
  uint face_id
) {
  face_attribute f;
...
face_texcoord read_face_texcoord(
  uint mesh_id,
  uint primitive_id
) {
  const mesh_type mesh = mesh_pool[ mesh_id ];
  const uvec3 vertex_index = read_face_index( mesh_id, primitive_id );
  face_texcoord texcoord;
  for( uint i = 0u; i != 3u; ++i ) {
    texcoord.texcoord[ i ] = read_vertex( accessor_pool[ mesh.accessor + 4 ], vertex_index[ i ], vec4( 0.0, 0.0, 0.0, 1.0 ) ).xy;
  }
  texcoord.vertex_index = vertex_index;
  return texcoord;
}

face_attribute read_face_attribute_without_texcoord(
  uint mesh_id,
  face_texcoord texcoord
) {
  const mesh_type mesh = mesh_pool[ mesh_id ];
  ...
  for( uint i = 0u; i != 3u; ++i ) {
    if( normal_accessor.enabled != 0 ) {
      f.vertex[ i ].normal = read_vertex( normal_accessor, texcoord.vertex_index[ i ], vec4( 0.0, 0.0, 0.0, 1.0 ) );
      if( tangent_accessor.enabled != 0 ) {
        f.vertex[ i ].tangent = read_vertex( tangent_accessor, texcoord.vertex_index[ i ], vec4( 0.0, 0.0, 0.0, 1.0 ) );
      }
      else if( normal_accessor.enabled != 0 ) {
        if( normal_accessor.type == GCT_SHADER_TYPE_ID_N21T11 ) {
          f.vertex[ i ].tangent = vec4( n21t11_decode_tangent( vertex_buffer_u32[ normal_accessor.vertex_buffer ].data[ normal_accessor.offset + texcoord.vertex_index[ i ] * normal_accessor.stride ], f.vertex[ i ].normal.xyz ), 1.0 );
        }
        else if( normal_accessor.type == GCT_SHADER_TYPE_ID_N20T11B1 ) {
          f.vertex[ i ].tangent = n20t11b1_decode_tangent( vertex_buffer_u32[ normal_accessor.vertex_buffer ].data[ normal_accessor.offset + texcoord.vertex_index[ i ] * normal_accessor.stride ], f.vertex[ i ].normal.xyz );
        }
        else {
          gen_tangent = true;
        }
    ...
    f.vertex[ i ].tex_coord0 = vec4( texcoord.texcoord[ i ], 0.0, 0.0 );
    f.vertex[ i ].color0 = read_vertex( accessor_pool[ mesh.accessor + 5 ], texcoord.vertex_index[ i ], vec4( 1.0, 1.0, 1.0, 1.0 ) );
    f.vertex[ i ].joint0 = read_vertex( accessor_pool[ mesh.accessor + 6 ], texcoord.vertex_index[ i ], vec4( 0.0, 0.0, 0.0, 0.0 ) );
    f.vertex[ i ].weight0 = read_vertex( accessor_pool[ mesh.accessor + 7 ], texcoord.vertex_index[ i ], vec4( 0.0, 0.0, 0.0, 0.0 ) );
    f.vertex[ i ].lod_morph = read_vertex( accessor_pool[ mesh.accessor + 8 ], texcoord.vertex_index[ i ], vec4( 0.0, 0.0, 0.0, 1.0 ) );
  }

  if( gen_normal ) {
    f.vertex[ 0 ].normal = vec4( 0.0, 1.0, 0.0, 1.0 );
    f.vertex[ 1 ].normal = vec4( 0.0, 1.0, 0.0, 1.0 );
    f.vertex[ 2 ].normal = vec4( 0.0, 1.0, 0.0, 1.0 );
  }

  if( gen_tangent ) {
    f.vertex[ 0 ].tangent = vec4( 1.0, 0.0, 0.0, 1.0 );
    f.vertex[ 1 ].tangent = vec4( 1.0, 0.0, 0.0, 1.0 );
    f.vertex[ 2 ].tangent = vec4( 1.0, 0.0, 0.0, 1.0 );
  }
  f.valid = true;
  return f;
}

face_attribute read_face_attribute( uint mesh_id, uint primitive_id ) {
  face_attribute f;
  uvec3 vertex_index = read_face_index( mesh_id, primitive_id );
  const mesh_type mesh = mesh_pool[ mesh_id ];
  const accessor_type normal_accessor = accessor_pool[ mesh.accessor + 2 ];
  const accessor_type tangent_accessor = accessor_pool[ mesh.accessor + 3 ];
  for( uint i = 0u; i != 3u; ++i ) {
...
/*face_attribute read_face_attribute( meshlet_reader reader, uint face_id ) {
  ...
  face_attribute f = read_face_position2( reader, face_id );
  read_face_texcoord( reader.mesh_id, f );
  read_face_attribute_without_texcoord( reader.mesh_id, f );
  return f; 
}*/
/*face_attribute read_face_attribute( meshlet_reader reader, uint face_id ) {
  if( !reader.valid ) return null_face_attr;
  face_attribute f;
  read_face_position( reader, face_id, f );
  read_face_texcoord( reader.mesh_id, f );
  read_face_attribute_without_texcoord( reader.mesh_id, f );
  return f; 
}*/
