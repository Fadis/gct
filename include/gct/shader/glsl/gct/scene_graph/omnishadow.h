#ifndef GCT_SHADER_SCENE_GRAPH_OMNISHADOW_H
#define GCT_SHADER_SCENE_GRAPH_OMNISHADOW_H

#include <gct/constants.h>
#include <gct/poisson_disk.h>
...
float pcss_omni( uint shadow_map, vec3 pos, vec3 light_pos, float light_size ) {
  ...
  for( int i = 0; i < poisson_disk_sample_count; i++ ) {
    ...
    float occluder_distance = texture( texture_pool_cube[ nonuniformEXT( shadow_map ) ], p + offset ).r;
...
float pcf_omni( uint shadow_map, vec3 pos, vec3 light_pos, float light_size ) {
  vec3 p = pos - light_pos;
  mat2 rot2 = get_rotate_matrix2( rand1( p.xy ) * 2 * pi );
  mat3 rot3 = get_rotate_matrix3( p );
  float receiver_distance = length( p );
  ...
  for( int i = 0; i < poisson_disk_sample_count; i++ ) {
    vec3 offset = rot3 * vec3( ( rot2 * poisson_disk[ i ] ) * light_size, 0.0 );
    float occluder_distance = texture( texture_pool_cube[ nonuniformEXT( shadow_map ) ], p + offset ).x;
    sum += ( occluder_distance < receiver_distance ) ? 0 : 1;
  }
  return sum / poisson_disk_sample_count;
}

float simple_shadow_omni( uint shadow_map, vec3 pos, vec3 light_pos ) {
  vec3 p = pos - light_pos;
  float distance = length( p );
  float projected = texture( texture_pool_cube[ nonuniformEXT( shadow_map ) ], p ).x;
