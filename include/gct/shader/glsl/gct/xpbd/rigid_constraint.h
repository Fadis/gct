...
rigid_collision_dx_dq rigid_collision_constraint_dx(
  ...
  if( dot( n0, n1 ) >= 0.0 ) is_end == true;
  const float scale = 0.05;
  const float friction_scale = 0.02;
  const float c = dot( n1, p1 - p0 ) * scale;
  if( c < 0.0 ) is_end = true;
  const vec3 n = n1;
  const float w0 = inv_m0 + dot( cross( r0, n1 ), inv_i0 * cross( r0, n1 ) );
  const float w1 = inv_m1 + dot( cross( r1, n0 ), inv_i1 * cross( r1, n0 ) );
  const float dl = ( -c ) / ( w0 + w1 );
  vec3 p = inv_m0 * n * dl;
  const vec3 relative_velocity = ( p0 - p_prev0 ) - ( p1 - p_prev1 );
  const vec3 friction = ( relative_velocity - dot( relative_velocity, n ) * n ) * friction_scale;
  vec3 dx = p + friction;
  vec4 dq = 1.0/2.0 * quaternion_quaternion_mult( vec4( inv_i0 * cross( r0, n * dl ), 0.0 ), q0 );
  dx = is_end ? vec3( 0, 0, 0 ) : dx;
  dq = is_end ? vec4( 0, 0, 0, 0 ) : dq;
  return rigid_collision_dx_dq(
    dx, dq, dx == vec3( 0, 0, 0 ) ? 0u : 1u, dq == vec4( 0, 0, 0, 0 ) ? 0u : 1u
  );
}
...
rigid_collision_dx_dq rigid_border_solve(
  ...
  const float dlf = 0.02;//( min( c, 0.003 ) ) / ( w0 );
...
rigid_collision_dx_dq rigid_border_dx(
  ...
  if( p0.x > aabb.max.x ) {
    ...
    rigid_collision_dx_dq d = rigid_border_solve( p0, p_prev0, q0, n0, r0, inv_m0, inv_i0, p1, n1, dt );
    dx += d.dx;
    dq += d.dq, dq;
    dx_count += d.dx_count;
    dq_count += d.dq_count;
  ...
  if( p0.y > aabb.max.y ) {
    ...
    const vec3 n1 = vec3( 0.0, -1.0, 0.0 );
    rigid_collision_dx_dq d = rigid_border_solve( p0, p_prev0, q0, n0, r0, inv_m0, inv_i0, p1, n1, dt );
    dx += d.dx;
    dq += d.dq;
    dx_count += d.dx_count;
  ...
  if( p0.z > aabb.max.z ) {
    ...
    const vec3 n1 = vec3( 0.0, 0.0, -1.0 );
    rigid_collision_dx_dq d = rigid_border_solve( p0, p_prev0, q0, n0, r0, inv_m0, inv_i0, p1, n1, dt );
    dx += d.dx;
    dq += d.dq;
    dx_count += d.dx_count;
