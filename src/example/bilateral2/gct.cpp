...
int main( int argc, const char *argv[] ) {
  ...
  gct::gbuffer depth_gbuffer(
    gct::gbuffer_create_info()
      .set_allocator( res.allocator )
      .set_width( res.width/2 )
      .set_height( res.height/2 )
      .set_layer( 1u )
      .set_swapchain_image_count( 1u )
  ...
  gct::lens_flare flare(
    gct::lens_flare_create_info()
      .set_allocator_set( res.allocator_set )
      .set_width( res.width / 16 )
      .set_height( res.height / 16 )
  ...
  const auto bilateral_weight_desc = builder.call(
    builder.get_image_io_create_info(
      std::make_shared< gct::compute >(
        gct::compute_create_info()
          .set_allocator_set( res.allocator_set )
          .set_shader( CMAKE_CURRENT_BINARY_DIR "/bilateral_weight/bilateral_weight.comp.spv" )
          .set_scene_graph( sg->get_resource() )
          .add_resource( { "global_uniforms", global_uniform } )
      ),
      gct::image_io_plan()
        .add_input( "src" )
        .add_input( "depth" )
        .add_output( "dest", "src", { 0.5f, -1.f }, vk::Format::eR8Uint )
        .set_dim( "src", { 0.5f, -1.f } )
        .set_node_name( "bilateral_weight" )
    )
  )(
  ...
  const auto lighting_desc = builder.call(
    builder.get_image_io_create_info(
      ...
      gct::image_io_plan()
        .add_input( "src" )
        .add_input( "depth" )
        .add_output( "dest", "src", { 0.5f, -4.f }, vk::Format::eR16G16B16A16Sfloat )
        .set_dim( "src", { 0.5f, -4.f } )
  ...
  const auto np_desc = builder.call(
    ...
    gct::image_io_plan()
      .add_input( "src" )
      .add_input( "depth" )
      .add_output( "dest", "src", glm::vec2( 0.25f, -1.f ), vk::Format::eR32Sfloat )
      .set_dim( "src", glm::vec2( 0.25f, -1.f ) )
  ...
  const auto mix_ao_desc = builder.call(
    ...
    gct::image_io_plan()
      ...
      .add_output( "coc_image", "gbuffer", glm::vec2( 0.5f, -2.0f ), vk::Format::eR16G16B16A16Sfloat )
      .add_output( "dest_image", "gbuffer", glm::vec2( 0.5f, -2.0f ), vk::Format::eR16G16B16A16Sfloat )
      .set_dim( "gbuffer", glm::vec2( 0.5f, -1.0f ) )
...

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      recorder.set_image_layout( bloom_view, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( bilateral_weight_view, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( sb.get_image_view(), vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }
  
  auto gamma = gct::compute(
    gct::compute_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/gamma/gamma.comp.spv" )
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "src_image", merged_view, vk::ImageLayout::eGeneral } )
      .add_resource( { "bloom_image", bloom_view, vk::ImageLayout::eGeneral } )
      .add_resource( { "dest_image", res.swapchain_image_views, vk::ImageLayout::eGeneral } )
      .add_resource( { "bilateral_weight", bilateral_weight_view, vk::ImageLayout::eGeneral } )
  );
  ...
  while( !walk.end() ) {
    ...
    {
      {
        ...
        glm::ivec2 focus( res.width/4, res.height/4 );
    ...
    auto &fb = framebuffers[ image_index ];
    {
      ...
      gamma( rec, image_index, res.width, res.height, 1u );
      rec.convert_image(
        res.swapchain_images[ image_index ],
