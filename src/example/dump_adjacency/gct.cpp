...
int main( int argc, const char *argv[] ) {
  ...
  std::cout << "unique vertex count : " << il0_prim->mesh.unique_vertex_count << std::endl;
  std::cout << "vertex count : " << il0_prim->mesh.vertex_count << std::endl;

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      same_position( recorder, 0, il0_prim->mesh.unique_vertex_count, il0_prim->mesh.unique_vertex_count, 1u );
      recorder.barrier( sg->get_resource()->vertex_to_primitive->get_buffer() );
      vertex_to_primitive( recorder, 0, il0_prim->mesh.vertex_count / 3, 1u, 1u );
      recorder.barrier( sg->get_resource()->vertex_to_primitive->get_buffer() );
      generate_adjacency( recorder, 0, il0_prim->mesh.vertex_count / 3, 1u, 1u );
    }
    command_buffer->execute_and_wait();
  }


  for( std::uint32_t adj_index = 0u; adj_index != 64u; ++adj_index ) {
    sg->get_resource()->adjacency->get(
      il0_prim->descriptor.adjacency,
      adj_index,
      [adj_index]( vk::Result r, std::vector< std::uint8_t > &&v ) {
        const std::uint32_t adj = *reinterpret_cast< std::uint32_t* >( v.data() );
        std::cout << adj << " ";
        if( ( adj_index + 1u ) % 3u == 0u ) {
          std::cout << std::endl;
        }
      }
    );
