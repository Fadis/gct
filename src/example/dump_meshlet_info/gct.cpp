...
int main( int argc, const char *argv[] ) {
  ...
  for( const auto &rp: il->get_draw_list() ) {
    ...
    for( unsigned int i = 0u; i != p->mesh.meshlet_count; ++i ) {
      std::cout << "request " <<  name << "." << i << std::endl;
      sg->get_resource()->meshlet->get(
        p->descriptor.meshlet,
        i,
        [i,name]( const vk::Result &result, std::vector< std::uint8_t > &&v ) {
          const auto &m = *reinterpret_cast< gct::meshlet_type* >( v.data() );
          std::cout << name << "." << i << " " << nlohmann::json( m ).dump() << std::endl;
        }
      );
    }
  }
