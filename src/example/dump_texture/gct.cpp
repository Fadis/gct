...
int main(
  ...
  const auto vs = device->get_shader_module( "./src/example/dump_texture/shader.vert.spv" );
  const auto fs = device->get_shader_module( "./src/example/dump_texture/shader.frag.spv" );
  ...
  std::cout << nlohmann::json( base_color_image->get_props() ).dump( 2 ) << std::endl;
    ...
    gct::image_view_create_info_t()
      .set_basic(
        vk::ImageViewCreateInfo()
          ...
          .setFormat( gct::add_srgb( base_color_image->get_props().get_basic().format ) )
      )
      .rebuild_chain()
  );
