...
struct physically_fog_parameters_t {
  LIBGCT_SETTER( g )
...
int main( int argc, const char *argv[] ) {
  ...
  const std::uint32_t pole_count = 256u;
  const std::uint32_t vertex_count = 32u;
  gct::epipolar_light_scattering epipolar_light_scattering(
    gct::epipolar_light_scattering_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_input( gbuffer.get_image_views() )
      .set_shadow( cubemap_images.get_cube_image_views() )
      .set_pole_count( pole_count )
      .set_vertex_count( vertex_count )
      .set_texel_per_sample( 32u )
      .add_draw_mesh_shader( CMAKE_CURRENT_BINARY_DIR "/epipolar/world.vert.spv" )
      .add_draw_mesh_shader( CMAKE_CURRENT_BINARY_DIR "/epipolar/world.frag.spv" )
      .set_generate_mesh2_shader( CMAKE_CURRENT_BINARY_DIR "/epipolar/generate2.comp.spv" )
      .set_generate_mesh3_shader( CMAKE_CURRENT_BINARY_DIR "/epipolar/generate3.comp.spv" )
      .set_generate_mesh4_shader( CMAKE_CURRENT_BINARY_DIR "/epipolar/generate4.comp.spv" )
      .set_volumetric_light_shader( CMAKE_CURRENT_BINARY_DIR "/volumetric_light/volumetric_light.comp.spv" )
      .set_clear_texcoord_shader( CMAKE_CURRENT_BINARY_DIR "/volumetric_light/clear_texcoord.comp.spv" )
      .set_generate_texcoord_shader( CMAKE_CURRENT_BINARY_DIR "/volumetric_light/texcoord.comp.spv" )
      .set_fill_texcoord_shader( CMAKE_CURRENT_BINARY_DIR "/volumetric_light/fill_texcoord.comp.spv" )
      .set_hgauss_shader( CMAKE_CURRENT_BINARY_DIR "/selective_gauss/h12_32.comp.spv" )
      .set_vgauss_shader( CMAKE_CURRENT_BINARY_DIR "/selective_gauss/v12_32.comp.spv" )
  );
  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      auto rec = command_buffer->begin();
      epipolar_light_scattering.init( rec );
    }
  ...
  auto mix_ao = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .add_resource( { "epipolar", epipolar_light_scattering.get_output(), vk::ImageLayout::eGeneral } )
  ...
  while( !walk.end() ) {
    ...
    const glm::vec4 light_pos_on_screen4 = projection * walk.get_lookat() * glm::vec4( walk.get_light_pos(), 1.0 );
    const glm::vec2 light_pos_on_screen = glm::vec2( light_pos_on_screen4[ 0 ]/light_pos_on_screen4[ 3 ], light_pos_on_screen4[ 1 ]/light_pos_on_screen4[ 3 ] );

    {
      ...
      epipolar_light_scattering(
        rec,
        image_index,
        gct::epipolar_light_scattering_parameters_t()
          .set_eye_pos( glm::vec4( walk.get_camera_pos(), 1.0 ) )
          .set_light_pos( glm::vec4( walk.get_light_pos(), 1.0 ) )
          .set_g( glm::vec4( 0.3, 0.5, 0.7, 1.0 ) )
          .set_fog_color( glm::vec4( 0.01, 0.01, 0.01, 1.0 ) )
          .set_light_energy( walk.get_light_energy() ),
        gct::epipolar_light_scattering_texcoord_parameters_t()
      ...
      {
        auto pc = physically_fog_parameters_t()
          .set_g( glm::vec4( 0.3, 0.5, 0.7, 1.0 ) )
          .set_fog_color( glm::vec4( 0.01, 0.01, 0.01, 1.0 ) );
        rec->pushConstants(
          **mix_ao.get_pipeline()->get_props().get_layout(),
          ...
          0u,
          sizeof( physically_fog_parameters_t ),
