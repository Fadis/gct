    ...
    {
      {
        ...
        if( frame_counter > 60 ) {
          ...
          il[ 2 ]->setup_resource_pair_buffer( rec );
          read_from_spatial_hash_fluid( rec, 0, il2_prim->mesh.unique_vertex_count * 27u, 1u, 1u );
          ...
          for( std::uint32_t i = 0u; i != 30u; ++i ) {
            ...
            il[ 2 ]->setup_resource_pair_buffer( rec );
            fluid_constraint( rec, 0, il2_prim->mesh.unique_vertex_count * 32u, 1u, 1u );
            rec.barrier( sg->get_resource()->particle->get_buffer() );
            rec.barrier( sg->get_resource()->lambda->get_buffer() );
          }
          ...
          rec.barrier( sg->get_resource()->particle->get_buffer() );
          fluid_viscosity( rec, 0, il2_prim->mesh.unique_vertex_count * 32u, 1u, 1u );
          rec.barrier( sg->get_resource()->particle->get_buffer() );
