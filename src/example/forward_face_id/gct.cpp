...
int main( int argc, const char *argv[] ) {
    ...
    builder.get_image_io_create_info(
      std::make_shared< gct::graphics >(
        gct::graphics_create_info()
          .set_swapchain_image_count( 1u )
          .add_shader( gct::get_system_shader_path() / "forward_rendering" / "face_id" / "geometry.task.spv" )
          .add_shader( gct::get_system_shader_path() / "forward_rendering" / "face_id" / "geometry.mesh.spv" )
          .add_shader( gct::get_system_shader_path() / "forward_rendering" / "face_id" / "geometry.frag.spv" )
  ...
  const glm::mat4 projection = glm::perspective( 0.6981317007977318f, (float(res.width)/float(res.height)), std::min(0.1f*scale,0.5f), scale * 2.0f );
