...
int main( int argc, const char *argv[] ) {
  ...
  const auto mix_ao = gct::compute(
    gct::compute_create_info()
      ...
      .add_resource( { "af_state", af_state_buffer } )
      .add_resource( { "dest_image", dof } )
      .add_resource( { "light_pool", sg->get_resource()->light->get_buffer() } )
      .add_resource( { "matrix_pool", sg->get_resource()->matrix->get_buffer() } )
  );
  
  const auto dof_hgauss = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/dof/hadaptive.comp.spv" )
      .set_swapchain_image_count( 1u )
      .add_resource( { "src_image", dof } )
      .add_resource( { "dest_image", dof_temporary } )
      .add_resource( { "coc", coc } )
  );

  const auto dof_vgauss = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/dof/vadaptive.comp.spv" )
      .set_swapchain_image_count( 1u )
    ...
    {
      {
        ...
        dof_hgauss( rec, 0, res.width, res.height, 1u );
        rec.compute_barrier( {}, { dof_temporary->get_factory() } );
        dof_vgauss( rec, 0, res.width, res.height, 1u );
