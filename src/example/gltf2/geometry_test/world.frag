...
void main()  {
  ...
  if( prim.base_color_texture != 0 ) {
    albedo = texture( texture_pool[ prim.base_color_texture ], input_texcoord );
  }
  else {
  ...
  if( prim.emissive_texture != 0 ) {
    emissive = texture( texture_pool[ prim.emissive_texture ], input_texcoord ).rgb;
  }
  else {
  ...
  if( prim.metallic_roughness_texture != 0 ) {
    vec4 mr = texture( texture_pool[ prim.metallic_roughness_texture ], input_texcoord );
    metallic = mr.b;
    roughness = mr.g;
  }
  else {
  ...
  if( prim.occlusion_texture != 0 ) {
    occlusion = mix( 1 - prim.occlusion_strength, 1, texture( texture_pool[ prim.occlusion_texture ], input_texcoord ).r );
  }
  else {
    occlusion = 1.0;
  }
  vec3 optflow = vec3( 0.0, 0.0, 0.0 );
  float shadow_level0 = pcss_omni( input_position.xyz, global_uniforms.light_pos.xyz, global_uniforms.light_size );
  ...
  output_albedo = vec4( 1.0, 1.0, 1.0, 1.0 );//albedo;
