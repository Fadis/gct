...
int main( int argc, const char *argv[] ) {
  ...
  const auto extended_gbuffer_desc = sg->get_resource()->image->allocate(
    gct::image_allocate_info()
      .set_create_info(
        gct::image_create_info_t()
          .set_basic(
            gct::basic_2d_image( res.width / 2, res.height / 2 )
  ...
  update_af.set_push_constant( "focus_pos", glm::ivec2( res.width/4, res.height/4 ) );
  ...
  const auto unnormalized_sampler_desc = sg->get_resource()->sampler->allocate_unnormalized();
