...
struct global_uniforms_t {
  ...
  LIBGCT_SETTER( depth )
  LIBGCT_SETTER( height_map_tex )
  LIBGCT_SETTER( height_normal_tex )
  LIBGCT_SETTER( terrain_tex )
  glm::vec4 eye_pos;
  ...
  std::int32_t gbuffer;
  std::int32_t depth;
  std::uint32_t height_map_tex;
  std::uint32_t height_normal_tex;
  std::uint32_t terrain_tex;
};
...
int main( int argc, const char *argv[] ) {
  ...
  const auto sg = std::make_shared< gct::scene_graph::scene_graph >(
    gct::scene_graph::scene_graph_create_info()
      ...
      .add_master_shader( CMAKE_CURRENT_BINARY_DIR "/terrain" )
  ...
  gct::gbuffer shadow_gbuffer(
    gct::gbuffer_create_info()
      ...
      .set_clear_color( std::array< float, 4u >{ 1000.f, 1000.f, 1000.f, 1.0 } )
  );
  ...
  const auto height_map_desc = sg->get_resource()->image->allocate(
    gct::image_load_info()
      .set_filename( CMAKE_CURRENT_SOURCE_DIR "/height.jpg" )
      .set_usage( vk::ImageUsageFlagBits::eSampled )
      .set_enable_linear( false )
  ).normalized;
  const auto height_map_tex_desc = sg->get_resource()->texture->allocate(
    sg->get_resource()->sampler->allocate_linear(),
    height_map_desc
  );
  const auto height_normal_desc = sg->get_resource()->image->allocate(
    gct::image_load_info()
      .set_filename( CMAKE_CURRENT_SOURCE_DIR "/height_normal.jpg" )
      .set_usage( vk::ImageUsageFlagBits::eSampled )
      .set_enable_linear( false )
  ).normalized;
  const auto height_normal_tex_desc = sg->get_resource()->texture->allocate(
    sg->get_resource()->sampler->allocate_linear(),
    height_normal_desc
  );
  const auto terrain_desc = sg->get_resource()->image->allocate(
    gct::image_load_info()
      .set_filename( CMAKE_CURRENT_SOURCE_DIR "/grass.jpg" )
      .set_usage( vk::ImageUsageFlagBits::eSampled )
      .set_enable_linear( true )
  ).linear;
  const auto terrain_tex_desc = sg->get_resource()->texture->allocate(
    sg->get_resource()->sampler->allocate(
  ...
  gct::graphics terrain(
    gct::graphics_create_info()
      ...
      .set_swapchain_image_count( 1u )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/terrain/terrain.task.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/terrain/terrain.mesh.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/terrain/terrain.frag.spv" )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "global_uniforms", global_uniform } )
  );
  ...
  const glm::mat4 projection = glm::perspective( 0.6981317007977318f, (float(res.width)/float(res.height)), std::min(0.1f*scale,0.5f), std::max( scale, 1000.0f ) );
    ...
    {
      {
        ...
        if( res.force_geometry || walk.camera_moved() || walk.light_moved() ) {
          const auto global_data = global_uniforms_t()
            ...
            .set_height_map_tex( *height_map_tex_desc )
            .set_height_normal_tex( *height_normal_tex_desc )
            .set_terrain_tex( *terrain_tex_desc );
