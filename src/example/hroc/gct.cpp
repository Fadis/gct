...
int main( int argc, const char *argv[] ) {
  ...
  auto hroc = std::make_shared< gct::scene_graph::hierarchical_raster_occlusion_culling >(
    gct::hierarchical_raster_occlusion_culling_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/occlusion" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/aabb/world.vert.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/aabb/world.geom.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/aabb/world.frag.spv" )
      .set_width( res.width )
      .set_height( res.height )
      ...
      if( walk.camera_moved() ) {
        {
          ...
          rec.transfer_to_graphics_barrier( { global_uniform->get_buffer() }, {} );
          {
            ...
            (*hroc)(
              ...
              projection,
              walk.get_lookat(),
              glm::vec4( walk.get_camera_pos(), 1.0 )
            );
            rec.sync_to_host( sg->get_resource()->last_visibility );
            rec.sync_to_host( hroc->get_sort_input() );
            rec.sync_to_host( hroc->get_sort_output() );
            rec.sync_to_host( hroc->get_bvh() );
            rec.sync_to_host( hroc->get_visibility() );
          }
        }
        command_buffer->execute_and_wait();
        /*{
          const auto mapped = sg->get_resource()->last_visibility->map< std::uint32_t >();
        ...
        /*{
          const auto mapped = hroc->get_sort_output()->map< gct::key_value_t >();
          for( const auto &v: mapped ) {
            std::cout << "key=" << v.key << " value=" << v.value << std::endl;
          }
          std::cout << std::endl;
        }*/
        /*{
          const auto mapped = hroc->get_bvh()->map< gct::scene_graph::hierarchical_raster_occlusion_culling::node_type >();
          const auto v_mapped = hroc->get_visibility()->map< std::uint32_t >();
          unsigned int i = 0u;
          for( const auto &v: mapped ) {
            std::cout << "cur=" << i << " left=" << v.left << " right=" << v.right << " parent=" << v.parent << " leaf=" << v.leaf << " visibility=" << v_mapped[ i ] << " aabb=" << nlohmann::json( v.aabb ) << std::endl;
            ++i;
          }
          std::cout << std::endl;
        }*/
