...
int main( int argc, const char *argv[] ) {
  ...
  const auto wvs = device->get_shader_module( "./src/example/images/shader.vert.spv" );
  const auto dfs = device->get_shader_module( "./src/example/images/global.frag.spv" );
  ...
  std::shared_ptr< gct::image_t > image;
  {
    auto command_buffer = queue->get_command_pool()->allocate();
    {
      auto recorder = command_buffer->begin();
        ...
        gct::image_view_create_info_t()
          .set_basic(
            vk::ImageViewCreateInfo()
              ...
              .setViewType( gct::to_image_view_type( image->get_props().get_basic().imageType, image->get_props().get_basic().arrayLayers ) )
  ...
  while( 1 ) {
    ...
    bool executed = false;
    {
      sync.command_buffer->wait_for_executed();
      {
        ...
        window.reset();
        float w = image->get_props().get_basic().extent.width*( 0.5f+std::sin( time )*0.5f+0.5f );
        float h = image->get_props().get_basic().extent.height*( 0.5f+std::cos( time * 3.1f )*0.5f+0.5f );
        window.add_rectangle(
          vk::Rect2D{
            vk::Offset2D{
              static_cast< std::int32_t >( width - w )/2,
              static_cast< std::int32_t >( height - h )/2
            },
            vk::Extent2D{ static_cast< std::uint32_t >( w ), static_cast< std::uint32_t >( h ) }
          },
