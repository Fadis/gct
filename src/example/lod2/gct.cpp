...
int main( int argc, const char *argv[] ) {
  ...
  const auto all_lods = std::make_shared< gct::scene_graph::instance_list >(
...

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      all_lods->setup_resource_pair_buffer( recorder );
      generate_meshlet_info( recorder, 0u, all_lods->get_mesh_count(), all_lods->get_max_primitive_count(), 1u );
    }
    command_buffer->execute_and_wait();
  }

  gct::gbuffer morph_gbuffer(
    gct::gbuffer_create_info()
      .set_allocator( res.allocator )
      .set_width( 512u )
      .set_height( 512u )
  ...
  const auto morph_gbuffer_desc = sg->get_resource()->image->allocate(
    gct::image_allocate_info()
      .set_create_info(
        gct::image_create_info_t()
          .set_basic(
            gct::basic_2d_image( 512u, 512u )
              .setUsage(
                vk::ImageUsageFlagBits::eStorage |
                vk::ImageUsageFlagBits::eTransferDst |
                vk::ImageUsageFlagBits::eTransferSrc
  ...
  const auto morph_voronoi_desc = sg->get_resource()->image->allocate(
    gct::image_allocate_info()
      .set_create_info(
        gct::image_create_info_t()
          .set_basic(
  ...
  const auto morph_filled_desc = sg->get_resource()->image->allocate(
    gct::image_allocate_info()
      .set_create_info(
        gct::image_create_info_t()
          .set_basic(
  ...
  const auto morph_sampler_desc = sg->get_resource()->sampler->allocate_linear();

  const auto morph_tex_desc = sg->get_resource()->texture->allocate(
    sg->get_resource()->sampler->allocate_linear(),
    morph_filled_desc
  );
  ...
  auto morph = gct::graphics(
    gct::graphics_create_info()
      .set_pipeline_create_info(
        gct::graphics_pipeline_create_info_t()
          .set_gbuffer( morph_gbuffer )
      )
      .set_swapchain_image_count( 1u )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/morph/morph.task.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/morph/morph.mesh.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/morph/morph.frag.spv" )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "global_uniforms", global_uniform } )
  );
  
  auto morph_mask = gct::compute(
    gct::compute_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/morph/mask.comp.spv" )
      .set_swapchain_image_count( 1u )
      .set_scene_graph( sg->get_resource() )
  );
  morph_mask.set_push_constant( "src", *morph_gbuffer_desc.linear );
  morph_mask.set_push_constant( "dest", *morph_voronoi_desc.linear );
  
  auto morph_voronoi = gct::compute(
    gct::compute_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/morph/voronoi.comp.spv" )
      .set_swapchain_image_count( 1u )
      .set_scene_graph( sg->get_resource() )
  );
  morph_voronoi.set_push_constant( "dest", *morph_voronoi_desc.linear );

  auto morph_fill = gct::compute(
    gct::compute_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/morph/fill.comp.spv" )
      .set_swapchain_image_count( 1u )
      .set_scene_graph( sg->get_resource() )
  );
  morph_fill.set_push_constant( "src", *morph_gbuffer_desc.linear );
  morph_fill.set_push_constant( "voronoi", *morph_voronoi_desc.linear );
  morph_fill.set_push_constant( "dest", *morph_filled_desc.linear );
  
  auto morph_generate = gct::compute(
    gct::compute_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/morph/generate.comp.spv" )
      .set_swapchain_image_count( 1u )
      .set_scene_graph( sg->get_resource() )
  );
  morph_generate.set_push_constant( "position", *morph_tex_desc.linear );

  const auto morph_vertex_buffer = doc2.get_morph_vertex_buffer();

  std::vector<
    std::pair<
      std::vector< gct::scene_graph::resource_pair >::const_iterator,
      std::vector< gct::scene_graph::resource_pair >::const_iterator
    >
  > generate_morph;

  {
    const auto &dl = all_lods->get_draw_list();
    auto higher = dl.begin();
    for( auto iter = dl.begin(); iter != dl.end(); ++iter ) {
      const auto inst = sg->get_resource()->inst.get( iter->inst );
      if( !inst->is_highest_lod ) {
        generate_morph.push_back( std::make_pair( higher, iter ) );
      }
      higher = iter;
    }
    ...
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      auto rec = command_buffer->begin();
      {
        const auto global_data = global_uniforms_t()
          .set_gbuffer( *morph_gbuffer_desc.linear );
        rec.copy( global_data, global_uniform );
        rec.transfer_to_graphics_barrier( global_uniform );
      }
      for( const auto &r: generate_morph ) {

        rec.fill( sg->get_resource()->image->get( morph_gbuffer_desc.linear )->get_factory(), gct::color::special::transparent );
        rec.fill( sg->get_resource()->image->get( morph_voronoi_desc.linear )->get_factory(), gct::color::special::transparent );
        rec.fill( sg->get_resource()->image->get( morph_filled_desc.linear )->get_factory(), gct::color::special::transparent );
        rec.barrier(
          gct::syncable()
            .add( sg->get_resource()->image->get( morph_gbuffer_desc.linear ) )
            .add( sg->get_resource()->image->get( morph_voronoi_desc.linear ) )
            .add( sg->get_resource()->image->get( morph_filled_desc.linear ) )
        );
        {
          auto render_pass_token = rec.begin_render_pass(
            morph_gbuffer.get_render_pass_begin_info( 0 ),
            vk::SubpassContents::eInline
          );
          rec->setViewport( 0, 1, &morph_gbuffer.get_viewport() );
          rec->setScissor( 0, 1, &morph_gbuffer.get_scissor() );
        ...
        morph_mask( rec, 0u, 512u, 512u, 1u );
        rec.barrier(
          gct::syncable()
            .add( sg->get_resource()->image->get( morph_voronoi_desc.linear ) )
        );
        for( std::uint32_t s = 256u; s != 0u; s >>= 1u ) {
          morph_voronoi.set_push_constant( "distance", s );
          morph_voronoi( rec, 0u, 512u, 512u, 8u );
          rec.barrier(
            gct::syncable()
              .add( sg->get_resource()->image->get( morph_voronoi_desc.linear ) )
          );
        }
        morph_fill( rec, 0u, 512u, 512u, 1u );
        rec.barrier(
          gct::syncable()
            .add( sg->get_resource()->image->get( morph_filled_desc.linear ) )
        );
        all_lods->for_each_unique_vertex(
          rec,
          morph_generate,
          r.first
        );
  ...
  const auto scale = std::max( std::abs( glm::length( scene_aabb->max - scene_aabb->min ) ), 10.f );
