...
int main( int argc, const char *argv[] ) {
  ...
  auto node = sg->get_root_node()->add_child(
...

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      recorder.set_image_layout( blured, vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }
  ...
  const auto mix_ao = gct::compute(
    gct::compute_create_info()
      ...
      .add_resource( { "specular_image", specular } )
      .add_resource( { "global_uniforms", global_uniform } )
      .add_resource( { "dest_image", mixed, vk::ImageLayout::eGeneral } )
      .add_resource( { "light_pool", sg->get_resource()->light->get_buffer() } )
      .add_resource( { "matrix_pool", sg->get_resource()->matrix->get_buffer() } )
  );
  
  const auto blur = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/blur/blur.comp.spv" )
      .set_swapchain_image_count( 1u )
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
      .add_resource( { "src_image", unnormalized_sampler, mixed, vk::ImageLayout::eShaderReadOnlyOptimal } )
      .add_resource( { "dest_image", blured, vk::ImageLayout::eGeneral } )
  );
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .set_input( std::vector< std::shared_ptr< gct::image_view_t > >{ blured } )
    ...
    {
      {
        ...
        blur( rec, 0, res.width, res.height, 1u );
