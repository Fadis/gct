...
int main( int argc, const char *argv[] ) {
  ...
  const auto vignetting_image = res.allocator->create_image_view(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY
  );

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      recorder.set_image_layout( vignetting_image, vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }
  ...
  const auto vignetting = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/vignetting/vignetting.comp.spv" )
      .set_swapchain_image_count( 1u )
      .add_resource( { "src_image", mixed } )
      .add_resource( { "dest_image", vignetting_image } )
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .set_input( std::vector< std::shared_ptr< gct::image_view_t > >{ vignetting_image } )
    ...
    {
      {
        ...
        vignetting( rec, 0, res.width, res.height, 1u );
        rec.compute_barrier( {}, { vignetting_image->get_factory() } );
        bloom_gauss( rec, 0 );
