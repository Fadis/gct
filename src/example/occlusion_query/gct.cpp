...
int main( int argc, const char *argv[] ) {
  ...
  auto occlusion_query = gct::occlusion_query< gct::scene_graph::resource_pair >(
    ...
    if( walk.get_current_camera() == 0 ) {
      if( walk.camera_moved() ) {
        ...
        for( std::uint32_t i = 0u; i != view_il->get_draw_list().size(); ++i ) {
          ...
          if( inst->initial_world_aabb && near_box ) {
            visible.push_back( view_il->get_draw_list()[ i ] );
          }
          else {
            occlusion_query.push( inst->initial_world_aabb, view_il->get_draw_list()[ i ] );
          }
        }
        {
          {
        ...
        occlusion_query.get_result( visible );
        view_il->get_draw_list() = visible;
      }
    }
    // rendering
