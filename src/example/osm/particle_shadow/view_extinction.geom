...
void main() {
  ...
  const vec4 camera_space_pos = dynamic_uniforms.camera_matrix * pos;
  ...
  vec4 screen_pos = dynamic_uniforms.projection_matrix * dynamic_uniforms.camera_matrix * pos;
  screen_pos /= screen_pos.w;
  // 線形な深度を求める
  const float linear_z = get_linear_z( screen_pos.z, push_constants.near, push_constants.far );
  // 線形な深度に基づいてどのレイヤーにビルボードを描くかを決める
  const uint layer = uint( push_constants.split_count * linear_z );
