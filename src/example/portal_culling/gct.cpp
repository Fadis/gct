...
int main( int argc, const char *argv[] ) {
  ...
  const float fovy = 0.6981317007977318f;
  const glm::mat4 projection = glm::perspective( fovy, (float(res.width)/float(res.height)), std::min(0.1f*scale,0.5f), scale );
  const float near = std::min(0.1f*scale,0.5f);
  const float far = scale;
  ...
  const gct::aabb cell_a = gct::aabb()
    .emplace_min( -14.f,  0.f, -14.f )
    .emplace_max(  14.f,  5.f,   0.f );
  const gct::aabb cell_b = gct::aabb()
    .emplace_min( -14.f,  0.f,   0.f )
    .emplace_max(  14.f,  5.f,  14.f );
  const std::vector< glm::vec4 > portal{
    { -1.f,  1.f, 0.f, 1.f },
    {  1.f,  1.f, 0.f, 1.f },
    { -1.f,  3.f, 0.f, 1.f },
    {  1.f,  3.f, 0.f, 1.f }
  ...
  while( !walk.end() ) {
    ...
    if( walk.camera_moved() ) {
      const auto np = gct::get_near_plane( projection, walk.get_camera()[ 0 ].get_lookat() );
      const auto fp = gct::get_far_plane( projection, walk.get_camera()[ 0 ].get_lookat() );
      float max_a_far = std::numeric_limits< float >::min();
      for( unsigned int i = 0u; i != 4u; ++i ) {
        const auto &[near,far] = gct::intersect(
          cell_a, np[ i ], fp[ i ]
        );
        max_a_far = std::max( max_a_far, far );
      }
      float min_b_near = std::numeric_limits< float >::max();
      for( unsigned int i = 0u; i != 4u; ++i ) {
        const auto &[near,far] = gct::intersect(
          cell_b, np[ i ], fp[ i ]
        );
        min_b_near = std::min( min_b_near, near );
      }
      min_b_near = std::max( min_b_near, 0.f );
      const float a_far = glm::mix( near, far, max_a_far );
      const float b_near = glm::mix( near, far, min_b_near );
      const glm::mat4 a_projection = glm::perspective( fovy, (float(res.width)/float(res.height)), near, a_far );
      const auto b_window = gct::get_minimum_projection_matrix(
        projection,
        walk.get_camera()[ 0 ].get_lookat(),
        portal,
        b_near
      );
      const auto b_projection = glm::frustum(
        b_window.min.x,
        b_window.max.x,
        b_window.min.y,
        b_window.max.y,
      ...
      draw_line.reset();
      for( const auto &v: gct::get_clippling_area_edges( a_projection, walk.get_camera()[ 0 ].get_lookat() ) ) {
        draw_line.push( v.first, v.second );
      }
      for( const auto &v: gct::get_clippling_area_edges( b_projection, walk.get_camera()[ 0 ].get_lookat() ) ) {
        draw_line.push( v.first, v.second );
      }
    }
    // frustum
