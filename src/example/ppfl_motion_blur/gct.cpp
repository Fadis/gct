...
int main( int argc, const char *argv[] ) {
  ...
  const auto fragment_list =
    res.allocator->create_buffer(
      sizeof( std::uint32_t ) * 32u * res.width * res.height,
  ...
  const auto generate_fragment_list = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/generate_fragment_list/generate_fragment_list.comp.spv" )
      .set_swapchain_image_count( 1u )
      .add_resource( { "gbuffer", extended_gbuffer, vk::ImageLayout::eGeneral } )
      .add_resource( { "fragment_list", fragment_list } )
  );
    ...
    {
      {
        ...
        {
          rec.fill( fragment_list, 0xfff );
          rec.barrier( { fragment_list }, {} );
          generate_fragment_list( rec, 0, res.width, res.height, 4u );
        ...
        mix_ao( rec, 0, res.width, res.height, 4u );
        rec.compute_barrier( { fragment_list }, { mixed->get_factory() } );
        bloom_gauss( rec, 0 );
