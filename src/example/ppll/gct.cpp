...
int main( int argc, const char *argv[] ) {
  ...
  constexpr std::size_t egbuf_count = 8u;
  ...
  const auto start_desc = sg->get_resource()->image->allocate(
    gct::image_allocate_info()
      .set_create_info(
        gct::image_create_info_t()
          .set_basic(
  ...
  const auto start = sg->get_resource()->image->get( start_desc.linear );

  const auto next_desc = sg->get_resource()->image->allocate(
    gct::image_allocate_info()
      .set_create_info(
        gct::image_create_info_t()
          .set_basic(
  ...
  const auto next = sg->get_resource()->image->get( next_desc.linear );

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      recorder.set_image_layout( start, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( next, vk::ImageLayout::eGeneral );
    }
  ...
  /*gct::lens_flare sb(
    gct::lens_flare_create_info()
      ...
      .set_shader( gct::get_system_shader_path() / "starburst" / "ppll" / "1.0" )
  ...
  /*gct::lens_flare flare(
    gct::lens_flare_create_info()
      ...
      .set_shader( gct::get_system_shader_path() / "lens_flare" / "prtlf" / "ppll" / "1.0" )
  ...
  gct::skyview_froxel2 skyview_froxel2(
    gct::skyview_froxel2_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( gct::get_system_shader_path() / "skyview" / "ppll" / "1.0" )
  ...
  gct::graphics geometry(
    gct::graphics_create_info()
      ...
      .set_swapchain_image_count( 1u )
      .add_shader( gct::get_system_shader_path() / "generate_ppll" / "standard" / "geometry.task.spv" )
      .add_shader( gct::get_system_shader_path() / "generate_ppll" / "standard" / "geometry.mesh.spv" )
      .add_shader( gct::get_system_shader_path() / "generate_ppll" / "standard" / "geometry.frag.spv" )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "global_uniforms", global_uniform } )
  );
  geometry.set_push_constant( "ppll_state_id", *ppll_state_desc );
  ...
  geometry.set_push_constant( "start", *start_desc.linear );
  geometry.set_push_constant( "next", *next_desc.linear );
  ...
  gct::graphics shadow(
    gct::graphics_create_info()
      ...
      .add_shader( gct::get_system_shader_path() / "generate_shadow" / "standard" / "geometry.task.spv" )
      .add_shader( gct::get_system_shader_path() / "generate_shadow" / "standard" / "geometry.mesh.spv" )
      .add_shader( gct::get_system_shader_path() / "generate_shadow" / "standard" / "geometry.frag.spv" )
      .set_scene_graph( sg->get_resource() )
  ...
  gct::graphics depth(
    gct::graphics_create_info()
      ...
      .add_shader( gct::get_system_shader_path() / "generate_depth" / "standard" / "geometry.task.spv" )
      .add_shader( gct::get_system_shader_path() / "generate_depth" / "standard" / "geometry.mesh.spv" )
      .add_shader( gct::get_system_shader_path() / "generate_depth" / "standard" / "geometry.frag.spv" )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "global_uniforms", shadow_uniform } )
  );
  ...
  const auto lighting_desc = builder.call(
    builder.get_image_io_create_info(
      std::make_shared< gct::compute >(
        gct::compute_create_info()
          .set_allocator_set( res.allocator_set )
          .set_shader( gct::get_system_shader_path() / "lighting" / "ppll" / "1.0" / "lighting.comp.spv" )
          .set_scene_graph( sg->get_resource() )
          .add_resource( { "global_uniforms", global_uniform } )
      ),
      gct::image_io_plan()
        ...
        .add_input( "start" )
        .add_input( "next" )
        .add_output( "dest", "gbuffer", { 1.f, -1.f }, vk::Format::eR16G16B16A16Sfloat )
    ...
    gct::shader_graph::vertex::combined_result_type()
      ...
      .add( "start", start_desc.linear )
      .add( "next", next_desc.linear )
  );
  ...
  const auto np_desc = builder.call(
    builder.get_image_io_create_info(
      std::make_shared< gct::compute >(
        gct::compute_create_info()
          .set_allocator_set( res.allocator_set )
          .set_shader( gct::get_system_shader_path() / "nearest_position" / "ppll" / "1.0" / "nearest_position.comp.spv" )
          .set_scene_graph( sg->get_resource() )
      ),
      gct::image_io_plan()
        .add_input( "gbuffer" )
        ...
        .add_input( "start" )
        .add_input( "next" )
        .add_output( "dest", "start", glm::vec2( 1.f, -1.f ), vk::Format::eR32Sfloat )
        .set_dim( "start", glm::vec2( 1.f, -1.f ) )
  ...
  const auto probe_desc = builder.call(
    builder.get_image_io_create_info(
      std::make_shared< gct::compute >(
        gct::compute_create_info()
          .set_allocator_set( res.allocator_set )
          .set_shader( gct::get_system_shader_path() / "probe_ppll" / "1.0" / "probe_ppll.comp.spv" )
          .set_scene_graph( sg->get_resource() )
      ),
      gct::image_io_plan()
        .add_input( "gbuffer" )
        ...
        .add_input( "start" )
        .add_input( "next" )
        .add_output( "dest", "start", glm::vec2( 1.f, -1.f ), vk::Format::eR32G32B32A32Sfloat )
        .set_dim( "start", glm::vec2( 1.f, -1.f ) )
        .set_node_name( "nearest_position" )
    )
    .set_push_constant( "ppll_state_id", *ppll_state_desc )
    .set_push_constant( "layer", 2u )
  )(
    gct::shader_graph::vertex::combined_result_type()
      .add( "gbuffer", extended_gbuffer_desc.linear )
      .add( "position", extended_depth_desc.linear )
  ...
  const auto decode_lighting_desc = builder.call(
    builder.get_image_io_create_info(
      std::make_shared< gct::compute >(
        gct::compute_create_info()
          .set_allocator_set( res.allocator_set )
          .set_shader( gct::get_system_shader_path() / "probe_ppll" / "1.0" / "probe_ppll_external.comp.spv" )
          .set_scene_graph( sg->get_resource() )
      ),
      gct::image_io_plan()
        .add_input( "gbuffer" )
  ...
  const auto mix_ao_desc = builder.call(
    builder.get_image_io_create_info(
      ...
      gct::image_io_plan()
        ...
        .add_output( "coc_image", "start", glm::vec2( 1.0f, -2.0f ), vk::Format::eR16G16B16A16Sfloat )
        .add_output( "dest_image", "start", glm::vec2( 1.0f, -2.0f ), vk::Format::eR16G16B16A16Sfloat )
        .set_dim( "start", glm::vec2( 1.0f, -1.0f ) )
  ...
  //builder.output( probe_desc[ "dest" ] );
  //builder.output( decode_lighting_desc[ "dest" ] );
  ...
  //const auto probe_view = compiled.get_view( probe_desc[ "dest" ] );
  //const auto lighting_view = compiled.get_view( decode_lighting_desc[ "dest" ] );
  ...
  auto update_af = gct::compute(
    gct::compute_create_info()
      ...
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( gct::get_system_shader_path() / "tone_mapping" / "ppll" / "1.0" / "distance.comp.spv" )
      .set_swapchain_image_count( 1u )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "af_state", af_state_buffer } )
  );
  update_af.set_push_constant( "ppll_state_id", *ppll_state_desc );
  ...
  update_af.set_push_constant( "start", *start_desc.linear );
  update_af.set_push_constant( "next", *next_desc.linear );
        ...
        if( res.force_geometry || walk.light_moved() || walk.camera_moved() ) {
          {
            ...
            rec.fill( extended_gbuffer->get_factory(), gct::color::special::transparent );
            rec.fill( start->get_factory(), gct::color::special::transparent );
            rec.fill( start->get_factory(), gct::color::special::transparent );
            rec.barrier(
              gct::syncable()
