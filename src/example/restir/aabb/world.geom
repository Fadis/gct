...
void main() {
  ...
  const mat4 projection_matrix = matrix_pool[ global_uniforms.projection_matrix ];
  const mat4 proj_cam = projection_matrix * matrix_pool[ global_uniforms.camera_matrix ];
  ...
  const float znear = projection_matrix[ 3 ][ 2 ] / ( -1.0 + projection_matrix[ 2 ][ 2 ] );
  aabb_type near_box;
