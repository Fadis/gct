...
int main( int argc, const char *argv[] ) {
  ...
  std::shared_ptr< gct::gbuffer > particle_gbuffer( new gct::gbuffer(
    gct::gbuffer_create_info()
      .set_allocator( res.allocator )
      .set_width( res.width )
      ...
      .set_external_depth( gbuffer.get_depth_views() )
  ) );

  gct::draw_particle draw_particle(
    gct::draw_particle_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_output( particle_gbuffer )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/particle/world.vert.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/particle/world.geom.spv" )
      .add_shader( CMAKE_CURRENT_BINARY_DIR "/particle/world.frag.spv" )
      .add_resource( { "dynamic_uniforms", dynamic_uniform } )
  );
  ...
  gct::particle_buffer particle_buffer(
    gct::particle_buffer_create_info()
      ...
      .set_vamap( draw_particle.get_vamap() )
      .set_stride( draw_particle.get_stride() )
  );
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .add_resource( { "smoke", particle_gbuffer->get_image_views(), vk::ImageLayout::eGeneral } ) 
  ...
  while( !walk.end() ) {
...

    {
      ...
      draw_particle( rec, image_index, particle_buffer );
      
      rec.convert_image(
        particle_gbuffer->get_image_views()[ image_index ]->get_factory(),
        vk::ImageLayout::eGeneral
      );
