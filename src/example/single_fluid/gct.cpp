...
int main( int argc, const char *argv[] ) {
  ...
  gct::graphics generate_fluid_depth(
    gct::graphics_create_info()
      ...
      .set_swapchain_image_count( 1u )
      .add_shader( gct::get_system_shader_path() / "draw_fluid" / "depth" / "geometry.task.spv" )
      .add_shader( gct::get_system_shader_path() / "draw_fluid" / "depth" / "geometry.mesh.spv" )
      .add_shader( gct::get_system_shader_path() / "draw_fluid" / "depth" / "geometry.frag.spv" )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "global_uniforms", global_uniform } )
      .add_resource( { "af_state", af_state_buffer } )
  );
  
  gct::gauss fluid_depth_gauss(
    gct::gauss_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( gct::get_system_shader_path() / "bilateral_filter" / "12_32_1" )
      .set_scene_graph( sg->get_resource() )
      .set_node_name( "fluid_depth_gauss" )
  );
  ...
  auto distance_constraint_dx = gct::compute(
    gct::compute_create_info()
      .set_allocator_set( res.allocator_set )
      .set_shader( gct::get_system_shader_path() / "xpbd" / "distance_constraint.comp.spv" )
      .set_swapchain_image_count( 1u )
      .set_scene_graph( sg->get_resource() )
      .add_resource( { "global_uniforms", global_uniform } )
  );
  ...
  const auto fluid_normal_desc = builder.call(
    builder.get_image_io_create_info(
      std::make_shared< gct::compute >(
        gct::compute_create_info()
          .set_allocator_set( res.allocator_set )
          .set_shader( gct::get_system_shader_path() / "draw_fluid" / "normal" / "normal.comp.spv" )
          .set_scene_graph( sg->get_resource() )
          .add_resource( { "global_uniforms", global_uniform } )
          .add_resource( { "af_state", af_state_buffer } )
  ...
  std::cout << nlohmann::json ( fluid_normal_view->get_factory()->get_props() ).dump( 2 ) << std::endl;
    ...
    {
      {
        ...
        if( frame_counter > 60 ) {
          ...
          il[ 1 ]->setup_resource_pair_buffer( rec );
          read_from_spatial_hash/*_fluid*/( rec, 0, il1_prim->mesh.unique_vertex_count * 27u, 1u, 1u );
          rec.barrier( sg->get_resource()->constraint->get_buffer() );

          for( std::uint32_t i = 0u; i != 30u; ++i ) {
            il[ 1 ]->setup_resource_pair_buffer( rec );
            fluid_constraint( rec, 0, il1_prim->mesh.unique_vertex_count * 32u, 1u, 1u );
            rec.barrier( sg->get_resource()->particle->get_buffer() );
            rec.barrier( sg->get_resource()->lambda->get_buffer() );
          }
          ...
          rec.barrier( sg->get_resource()->particle->get_buffer() );
          //fluid_viscosity( rec, 0, il1_prim->mesh.unique_vertex_count * 32u, 1u, 1u );
          rec.barrier( sg->get_resource()->particle->get_buffer() );
