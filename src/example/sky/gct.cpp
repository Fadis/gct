...
int main( int argc, const char *argv[] ) {
  ...
  const float light_energy = 20.f;//1366.f;
  const float g = 0.8f;
  const float sun_angle = M_PI/3.0;
  const glm::vec4 light_pos = glm::vec4( float( std::cos( sun_angle ) ), float( std::sin( sun_angle ) ), 0.f, 0.f ) * 14960000000.0f;
  const glm::vec4 light_pos_near = glm::vec4( glm::vec3( float( std::cos( sun_angle ) ), float( std::sin( sun_angle ) ), 0.f ) * 149.6f, 1.0 );
  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
