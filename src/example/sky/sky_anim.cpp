...
int main( int argc, const char *argv[] ) {
  ...
  while( !walk.end() ) {
...

    {
      ...
      {
        sun_angle += 0.003f;
        light_pos = glm::vec4( float( std::cos( sun_angle ) ), float( std::sin( sun_angle ) ), 0.f, 0.f ) * 14960000000.0f;
        light_pos_near = glm::vec4( glm::vec3( float( std::cos( sun_angle ) ), float( std::sin( sun_angle ) ), 0.f ) * 149.6f, 1.0 );
        float altitude = 6360000.f - walk.get_camera_pos().y;
        ++walk;
