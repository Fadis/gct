...
int main( int argc, const char *argv[] ) {
  ...
  const auto ao_out = res.allocator->create_image_views(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY,
    res.swapchain_images.size()
  );
  ...
  const auto ao = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/ao/ssdo_with_bounce.comp.spv" )
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
      .add_resource( { "dest_image", ao_out } )
      .add_resource( { "diffuse_image", diffuse } )
      .add_resource( { "dynamic_uniforms", dynamic_uniform } )
      .add_resource( { "environment_map", environment_sampler, environment_image_view } )
  );

  const auto ao_gauss = gct::image_filter(
    gct::image_filter_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/selective_gauss/h12_32.comp.spv" )
      .set_input( ao_out )
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
  )(
    gct::image_filter_create_info()
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/selective_gauss/v12_32.comp.spv" )
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
  );
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } ) 
      .add_resource( { "specular_image", specular } ) 
      .add_resource( { "occlusion", ao_gauss.get_output(), vk::ImageLayout::eGeneral } )
  ...
  while( !walk.end() ) {
...

    {
      ...
      ao( rec, image_index, res.width, res.height, 1u );
