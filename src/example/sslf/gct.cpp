...
int main( int argc, const char *argv[] ) {
  ...
  const auto mixed = res.allocator->create_image_view(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY
  );

  const auto flare_image = res.allocator->create_image_view(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY
  );
  const auto flare_temporary = res.allocator->create_image_view(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY
  );
...

  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      recorder.set_image_layout( mixed, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( flare_image, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( flare_temporary, vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }
  ...
  const auto flare = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/flare/flare.comp.spv" )
      .set_swapchain_image_count( 1u )
      .add_resource( { "src_image", unnormalized_sampler, mixed, vk::ImageLayout::eShaderReadOnlyOptimal } )
      .add_resource( { "dest_image", flare_image } )
  );

  const auto flare_hgauss = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      ...
      .set_swapchain_image_count( 1u )
      .add_resource( { "src_image", flare_image } )
      .add_resource( { "dest_image", flare_temporary } )
  );

  const auto flare_vgauss = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      ...
      .set_swapchain_image_count( 1u )
      .add_resource( { "src_image", flare_temporary } )
      .add_resource( { "dest_image", flare_image } )
  );
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .set_input( std::vector< std::shared_ptr< gct::image_view_t > >{ mixed } )
      .set_output_create_info( rgba32ici )
      .set_output_name( "bloom_image" )
      .add_resource( { "flare_image", flare_image } )
    ...
    {
      {
        ...
        rec.compute_barrier( {}, { mixed->get_factory() } );
        rec.convert_image( mixed->get_factory(), vk::ImageLayout::eShaderReadOnlyOptimal );
        flare( rec, 0, res.width, res.height, 1u );
        rec.convert_image( mixed->get_factory(), vk::ImageLayout::eGeneral );
        flare_hgauss( rec, 0, res.width, res.height, 1u );
        rec.compute_barrier( {}, { flare_temporary->get_factory() } );
        flare_vgauss( rec, 0, res.width, res.height, 1u );
        rec.compute_barrier( {}, { flare_image->get_factory() } );
        bloom_gauss( rec, 0 );
