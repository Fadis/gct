...
int main( int argc, const char *argv[] ) {
  ...
  const auto r32qici =
    gct::image_create_info_t()
      .set_basic(
        gct::basic_2d_image( res.width/4u, res.height/4u )
          .setFormat( vk::Format::eR32Sfloat )
          .setUsage( vk::ImageUsageFlagBits::eStorage )
      );
  const auto rgba32qici =
    gct::image_create_info_t()
      .set_basic(
        gct::basic_2d_image( res.width/4u, res.height/4u )
          .setUsage(
  ...
  const auto direct = res.allocator->create_image_views(
    r32qici,
    VMA_MEMORY_USAGE_GPU_ONLY,
    res.swapchain_images.size()
  );
  const auto qpos = res.allocator->create_image_views(
    rgba32qici,
    VMA_MEMORY_USAGE_GPU_ONLY,
    res.swapchain_images.size()
  );
  ...
  );
  {
    auto command_buffer = res.queue->get_command_pool()->allocate();
    {
      ...
      recorder.set_image_layout( qpos, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( scattering, vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }

  const auto calc_direct = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/direct_light/direct_light.comp.spv" )
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "src_image", qpos } )
      .add_resource( { "dest_image", direct } )
      .add_resource( { "dynamic_uniforms", dynamic_uniform } )
  );

  const auto epipolar_blur = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/epipolar_blur/epipolar_blur.comp.spv" )
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "src_image", direct } )
      .add_resource( { "dest_image", scattering } )
      .add_resource( { "dynamic_uniforms", dynamic_uniform } )
  );
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .add_resource( { "direct_image", scattering } )
  ...
  while( !walk.end() ) {
...

    {
      ...
      rec.graphics_to_transfer_barrier(
        {},
        { gbuffer.get_image( image_index ) }
      );
      ...
      calc_direct( rec, image_index, res.width / 4u, res.height / 4u, 1u );
      
      rec.compute_barrier(
        {},
        {
          direct[ image_index ]->get_factory()
        }
      );

      epipolar_blur( rec, image_index, res.width / 4u, res.height / 4u, 1u );
