...
int main() {
  ...
  const auto wvs = device->get_shader_module( "./src/example/text/shader.vert.spv" );
  const auto wfs = device->get_shader_module( "./src/example/text/shader.frag.spv" );
  const auto dfs = device->get_shader_module( "./src/example/text/global.frag.spv" );
        ...
        gct::image_view_create_info_t()
          .set_basic(
            vk::ImageViewCreateInfo()
              ...
              .setViewType( gct::to_image_view_type( icon->get_props().get_basic().imageType ) )
  ...
  window.add_rectangle(
    vk::Rect2D{ vk::Offset2D{ 0, 0 }, vk::Extent2D{ 1280, 300 } },
  ...
  const std::vector< std::uint32_t > glyphs{
    43, 72, 79, 79, 82, 15, 3, 90, 82, 85, 79, 71, 4
  };
  const std::vector< float > glyph_pos{
    0, 133.261, 234.683, 286.735, 338.787, 449.689, 500.49, 541.452, 687.592, 798.494, 869.507, 921.559, 1034.97
  };
  for( unsigned int i = 0u; i != glyphs.size(); ++i ) {
    window.add_glyph(
      0, glyphs[ i ], 10.f+glyph_pos[ i ], 10.f+13.356f, 183.167f,
      gct::srgb_oetf( gct::html_color( 0x000000 ) ),
      2u, 0u, 0u
    );
  }
  ...
  for( std::size_t i = 0u; i != swapchain_images.size(); ++i ) {
    ...
    desktop.back()->add_rectangle(
      vk::Rect2D{ vk::Offset2D{ ( width - 1280u ) / 2u, ( height - 300u ) / 2u }, vk::Extent2D{ 1280, 300 } },
      gct::srgb_oetf( gct::html_color( 0x000000 ) ),
      1u, true, 0u, 1u, 0u
    );
  }
