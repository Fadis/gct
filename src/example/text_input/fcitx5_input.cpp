...
int main( int argc, const char *argv[] ) {
  ...
  gct::input::fcitx5_preedit preedit;
  gct::input::fcitx5_client_ui client_ui;
  bool text_changed = false;
  std::mutex text_guard;
  thread_pool->add_co(
    [thread_pool=thread_pool,keyboard=keyboard,&current_text,&text_changed,&text_guard,&preedit,&client_ui]() {
      ...
      std::shared_ptr< gct::input::fcitx5 > fcitx5( new gct::input::fcitx5( thread_pool ) );
      fcitx5->run();
      thread_pool->add_co(
        [xkb_state=xkb_state,fcitx5=fcitx5]() {
          try {
            while( 1 ) {
      ...
      thread_pool->add_co(
        [fcitx5=fcitx5]() {
          try {
            while( 1 ) {
              auto f = fcitx5->get_current_im();
              auto [a,b,c] = gct::sched::wait( std::move( f ) );
              std::cout << "IM変更 : " << a << ", " << b << ", " << c << std::endl;
            }
          }
          catch( ... ) {
            std::cout << "im watcher exited." << std::endl;
          }
        }
      );
      thread_pool->add_co(
        [fcitx5=fcitx5,&current_text,&text_changed,&text_guard]() {
          try {
            while( 1 ) {
              for( auto c: gct::sched::wait( fcitx5->get_forwarded() ) ) {
                if( 0x20 <= c && c <= 0x7F ) {
                  ...
                  if( !current_text.empty() ) {
                    std::scoped_lock< std::mutex > lock( text_guard );
                    while( ( current_text.back() & 0xC0 ) == 0x80 ) {
                      std::cout << "pop " << int( current_text.back() ) << std::endl;
      ...
      thread_pool->add_co(
        [fcitx5=fcitx5,&current_text,&text_changed,&text_guard]() {
          try {
            while( 1 ) {
              auto s = gct::sched::wait( fcitx5->get_commited() );
              std::scoped_lock< std::mutex > lock( text_guard );
              current_text += s;
              text_changed = true;
            }
      ...
      thread_pool->add_co(
        [fcitx5=fcitx5,&preedit,&text_changed,&text_guard]() {
          try {
            while( 1 ) {
              auto s = gct::sched::wait( fcitx5->get_preedit() );
              std::scoped_lock< std::mutex > lock( text_guard );
              preedit = s;
              text_changed = true;
            }
      ...
      thread_pool->add_co(
        [fcitx5=fcitx5,&client_ui,&text_changed,&text_guard]() {
          try {
            while( 1 ) {
              auto s = gct::sched::wait( fcitx5->get_client_ui() );
              std::scoped_lock< std::mutex > lock( text_guard );
              client_ui = s;
              text_changed = true;
            }
      ...
      if( fb.initial || text_changed ) {
        {
          ...
          if( text_changed ) {
            ...
            float cursor_x = layouted.cursor_x;
            float cursor_y = layouted.cursor_y;
            window.reset();
            ...
            for( auto &v: preedit.cands ) {
              ...
              const auto line = engine.parse_lines( v.data(), v.size(), "ja-JP" );
              auto lines = engine.wrap_line( line );
              const auto layouted = engine.layout( lines, cursor_x, cursor_y );
              cursor_x = layouted.cursor_x;
              cursor_y = layouted.cursor_y;
              for( const auto &g: layouted.glyphs ) {
                auto y = g.y_pos+((height-40)+layouted.height);
                if( y > 0 ) {
                  std::cout << g.glyph_id << "(" << g.x_pos + 20 << "," << g.y_pos << ","  << g.size << ")" << " ";
                  window.add_glyph(
                    0, g.glyph_id, g.x_pos+20, y, g.size,
                    gct::srgb_oetf( gct::html_color( ( i == preedit.selected ) ? 0x0000FF : 0x8080FF ) ),
                    2u, 0u, 0u
                  );
                }
              }
