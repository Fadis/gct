...
struct light_scattering_parameters_t {
...
struct light_extinction_parameters_t {
  LIBGCT_SETTER( g )
  LIBGCT_SETTER( fog_color )
  glm::vec4 g;
  glm::vec4 fog_color;
...
int main( int argc, const char *argv[] ) {
  ...
  gct::gbuffer transparent_shadow_gbuffer(
    gct::gbuffer_create_info()
      .set_allocator( res.allocator )
      .set_width( shadow_map_size )
      .set_height( shadow_map_size )
  ...
  gct::cubemap_images2 transparent_cubemap_images( transparent_shadow_gbuffer.get_image_views() );
  ...
  gct::gbuffer transparent_front_gbuffer(
    res.allocator,
    res.width,
    res.height,
    res.swapchain_images.size(),
  ...
  gct::gbuffer transparent_back_gbuffer(
    gct::gbuffer_create_info()
      .set_allocator( res.allocator )
      .set_width( res.width )
  ...
  const auto dynamic_descriptor_set_layout = res.device->get_descriptor_set_layout(
    {       
      ...
      CMAKE_CURRENT_BINARY_DIR "/depth_only",
  ...
  const auto transparent_diffuse = res.allocator->create_image_views(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY,
    res.swapchain_images.size()
  );
  const auto transparent_specular = res.allocator->create_image_views(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY,
    res.swapchain_images.size()
  );

  {
    ...
    {
      auto recorder = command_buffer->begin();
      recorder.set_image_layout( transparent_diffuse, vk::ImageLayout::eGeneral );
      recorder.set_image_layout( transparent_specular, vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }

  const auto transparent_lighting = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/transparent_lighting/lighting.comp.spv" )
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "gbuffer", transparent_front_gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
      .add_resource( { "diffuse_image", transparent_diffuse } )
      .add_resource( { "specular_image", transparent_specular } )
      .add_resource( { "dynamic_uniforms", dynamic_uniform } )
  );
  ...
  const auto volumetric_light = res.allocator->create_image_views(
    rgba32ici,
    VMA_MEMORY_USAGE_GPU_ONLY,
    res.swapchain_images.size()
  );
  
  {
    ...
    {
      auto recorder = command_buffer->begin();
      recorder.set_image_layout( volumetric_light, vk::ImageLayout::eGeneral );
    }
    command_buffer->execute_and_wait();
  }
  
  const auto calc_volumetric_light = gct::compute(
    gct::compute_create_info()
      .set_allocator( res.allocator )
      .set_descriptor_pool( res.descriptor_pool )
      .set_pipeline_cache( res.pipeline_cache )
      .set_shader( CMAKE_CURRENT_BINARY_DIR "/volumetric_light/volumetric_light.comp.spv" )
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "gbuffer", transparent_front_gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
      .add_resource( { "back_gbuffer", transparent_back_gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
      .add_resource( { "dest_image", volumetric_light } )
      .add_resource( { "shadow", cubemap_sampler, cubemap_images.get_cube_image_views(), vk::ImageLayout::eShaderReadOnlyOptimal } )
      .add_resource( { "transparent_shadow", cubemap_sampler, transparent_cubemap_images.get_cube_image_views(), vk::ImageLayout::eShaderReadOnlyOptimal } )
  );
  ...
  auto mix_ao = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } ) 
      .add_resource( { "transparent_gbuffer", transparent_front_gbuffer.get_image_views(), vk::ImageLayout::eGeneral } ) 
      .add_resource( { "specular_image", specular } ) 
      .add_resource( { "transparent_specular_image", transparent_specular } ) 
      .add_resource( { "volumetric_light", volumetric_light } ) 
  ...
  gct::gltf::document_t transparent_doc;
  {
    const auto gcb = res.queue->get_command_pool()->allocate();
    {
      auto rec = gcb->begin();
      transparent_doc = gct::gltf::load_gltf(
        CMAKE_CURRENT_SOURCE_DIR "/bunny.gltf",
        res.device,
        rec,
        res.allocator,
        res.descriptor_pool,
        {
          transparent_shadow_gbuffer.get_render_pass(),
          transparent_front_gbuffer.get_render_pass(),
          transparent_back_gbuffer.get_render_pass()
        },
        {
          CMAKE_CURRENT_BINARY_DIR "/shadow",
          CMAKE_CURRENT_BINARY_DIR "/geometry",
  ...
  while( !walk.end() ) {
...

    {
      ...
      {
        auto render_pass_token = rec.begin_render_pass(
          transparent_shadow_gbuffer.get_render_pass_begin_info( image_index ),
          vk::SubpassContents::eInline
        );
        rec->setViewport( 0, 1, &transparent_shadow_gbuffer.get_viewport() );
        rec->setScissor( 0, 1, &transparent_shadow_gbuffer.get_scissor() );
        rec->setCullMode( vk::CullModeFlagBits::eBack );
        rec->setDepthCompareOp( vk::CompareOp::eLessOrEqual );
        gct::gltf::draw_node(
          rec,
          transparent_doc.node,
          transparent_doc.mesh,
          transparent_doc.buffer,
          0u,
          {
            shadow_descriptor_set[ image_index ]
          }
      ...
      rec.convert_image(
        transparent_shadow_gbuffer.get_image( image_index ),
        vk::ImageLayout::eShaderReadOnlyOptimal
      );
      rec.barrier(
      ...
      {
        auto render_pass_token = rec.begin_render_pass(
          transparent_front_gbuffer.get_render_pass_begin_info( image_index ),
          vk::SubpassContents::eInline
        );
        rec->setViewport( 0, 1, &transparent_front_gbuffer.get_viewport() );
        rec->setScissor( 0, 1, &transparent_front_gbuffer.get_scissor() );
        rec->setCullMode( vk::CullModeFlagBits::eBack );
        rec->setDepthCompareOp( vk::CompareOp::eLessOrEqual );
        gct::gltf::draw_node(
          rec,
      ...
      {
        auto render_pass_token = rec.begin_render_pass(
          transparent_back_gbuffer.get_render_pass_begin_info( image_index ),
          vk::SubpassContents::eInline
        );
        rec->setViewport( 0, 1, &transparent_back_gbuffer.get_viewport() );
        rec->setScissor( 0, 1, &transparent_back_gbuffer.get_scissor() );
        rec->setCullMode( vk::CullModeFlagBits::eFront );
        rec->setDepthCompareOp( vk::CompareOp::eGreaterOrEqual );
        gct::gltf::draw_node(
          rec,
          transparent_doc.node,
          transparent_doc.mesh,
      ...
      rec.convert_image(
        transparent_front_gbuffer.get_image( image_index ),
        vk::ImageLayout::eGeneral
      );
      rec.convert_image(
        transparent_back_gbuffer.get_image( image_index ),
        vk::ImageLayout::eGeneral
      );
      
      lighting( rec, image_index, res.width, res.height, 1u );
      
      transparent_lighting( rec, image_index, res.width, res.height, 1u );
      
      {
        auto params = light_scattering_parameters_t()
          .set_eye_pos( glm::vec4( walk.get_camera_pos(), 1.0 ) )
          .set_light_pos( glm::vec4( walk.get_light_pos(), 1.0 ) )
          .set_g( glm::vec4( 0.9, 0.8, 0.7, 1.0 ) )
          .set_fog_color( glm::vec4( 0.7f, 0.5f, 0.1f, 1.0f ) )
          .set_light_energy( walk.get_light_energy() );
        rec->pushConstants(
          **calc_volumetric_light.get_pipeline()->get_props().get_layout(),
          vk::ShaderStageFlagBits::eCompute,
          0u,
          sizeof( light_scattering_parameters_t ),
          reinterpret_cast< void* >( &params )
        );
      }

      calc_volumetric_light( rec, image_index, res.width, res.height, 1u );
      ...
      rec.compute_barrier(
        {},
        {
          ...
          transparent_diffuse[ image_index ]->get_factory(),
          transparent_specular[ image_index ]->get_factory(),
          volumetric_light[ image_index ]->get_factory()
        }
      );
      
      {
        auto params = light_extinction_parameters_t()
          .set_g( glm::vec4( 0.3, 0.25, 0.2, 1.0 ) )
          .set_fog_color( glm::vec4( 3.5f, 1.5f, 5.0f, 1.0f ) );
        rec->pushConstants(
          **mix_ao.get_pipeline()->get_props().get_layout(),
          vk::ShaderStageFlagBits::eCompute,
          0u,
          sizeof( light_extinction_parameters_t ),
          reinterpret_cast< void* >( &params )
        );
      }
