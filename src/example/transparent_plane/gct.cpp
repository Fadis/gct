...
int main( int argc, const char *argv[] ) {
  ...
  gct::gbuffer transparent_gbuffer(
    res.allocator,
    res.width,
    res.height,
    res.swapchain_images.size(),
  ...
  const auto transparent_lighting = gct::compute(
    gct::compute_create_info()
      ...
      .set_swapchain_image_count( res.swapchain_images.size() )
      .add_resource( { "gbuffer", transparent_gbuffer.get_image_views(), vk::ImageLayout::eGeneral } )
  ...
  const auto bloom_gauss = gct::image_filter(
    gct::image_filter_create_info()
      ...
      .add_resource( { "gbuffer", gbuffer.get_image_views(), vk::ImageLayout::eGeneral } ) 
      .add_resource( { "specular_image", specular } ) 
      .add_resource( { "transparent_gbuffer", transparent_gbuffer.get_image_views(), vk::ImageLayout::eGeneral } ) 
      .add_resource( { "transparent_specular_image", transparent_specular } ) 
      .add_resource( { "transparent_diffuse_image", transparent_diffuse } ) 
  ...
  gct::gltf::document_t transparent_doc;
  {
    const auto gcb = res.queue->get_command_pool()->allocate();
    {
      ...
      transparent_doc = gct::gltf::load_gltf(
        CMAKE_CURRENT_SOURCE_DIR "/plate.gltf",
        res.device,
        rec,
        res.allocator,
        res.descriptor_pool,
        {
          transparent_gbuffer.get_render_pass()
        },
        {
          CMAKE_CURRENT_BINARY_DIR "/geometry"
        },
  ...
  while( !walk.end() ) {
...

    {
      ...
      {
        auto render_pass_token = rec.begin_render_pass(
          transparent_gbuffer.get_render_pass_begin_info( image_index ),
          vk::SubpassContents::eInline
        );
        rec->setViewport( 0, 1, &transparent_gbuffer.get_viewport() );
        rec->setScissor( 0, 1, &transparent_gbuffer.get_scissor() );
        gct::gltf::draw_node(
          rec,
      ...
      rec.convert_image(
        transparent_gbuffer.get_image( image_index ),
        vk::ImageLayout::eGeneral
      );
