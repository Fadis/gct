...
int main( int argc, const char *argv[] ) {
  ...
  gct::image_pool::image_descriptor depth_desc;
  {
    ...
    const auto bg2_desc = builder.call(
      builder.get_image_io_create_info(
        std::make_shared< gct::graphics >(
          gct::graphics_create_info()
            .add_shader( gct::get_system_shader_path() / "generate_k+buffer" / "no_culling" / "2.0" )
    ...
    builder.output( bg2_desc[ "gbuffer" ] );
    builder.output( bg2_desc[ "position" ] );
    generate_gbuffer = std::make_shared< gct::shader_graph::compiled >( builder() );
    gbuffer_desc = generate_gbuffer->get_image_descriptor( bg2_desc[ "gbuffer" ] ); 
    depth_desc = generate_gbuffer->get_image_descriptor( bg2_desc[ "position" ] ); 
  ...
  {
    {
      ...
      generate_particle_radius( recorder, 0, il1_prim->mesh.unique_vertex_count, 1u, 1u );
      ...
      recorder.barrier( sg->get_resource()->vertex_to_primitive->get_buffer() );
      generate_particle_radius( recorder, 0, il2_prim->mesh.unique_vertex_count, 1u, 1u );
