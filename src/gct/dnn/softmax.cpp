...
softmax::softmax(
  ...
  std::string shader_name( "softmax_" );
  ...
  spec_t spec{
    ( output.dim.size() > 1u ) ?
      std::uint32_t( get_length( std::next( output.dim.begin() ), output.dim.end() ) ) :
  ...
  exec_dim[ 0 ] = 1024;
  exec_dim[ 1 ] = 1;
  exec_dim[ 2 ] = ( output.dim.size() > 1u ) ? output.dim[ 0 ] : 1;
}
void softmax::operator()( command_buffer_recorder_t &rec ) {
