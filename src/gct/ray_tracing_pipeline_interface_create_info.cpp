...
namespace gct {
  ...
  void from_json( const nlohmann::json &root, ray_tracing_pipeline_interface_create_info_t &v ) {
    if( !root.is_object() ) throw incompatible_json( "The JSON is incompatible to ray_tracing_pipeline_interface_create_info_t", __FILE__, __LINE__ );
    LIBGCT_EXTENSION_FROM_JSON( basic )
  }
  ray_tracing_pipeline_interface_create_info_t &ray_tracing_pipeline_interface_create_info_t::rebuild_chain() {
