...
namespace gct {
  ...
  std::optional< spv_member_pointer > shader_module_reflection_t::get_member_pointer_maybe( const std::string &name, memory_layout layout ) const {
    for( std::uint32_t i = 0u; i != reflect->descriptor_binding_count; ++i ) {
      if( reflect->descriptor_bindings[ i ].name == name ) {
        const auto descriptor_type = reflect->descriptor_bindings[ i ].descriptor_type;
        if(
          ...
          if( reflect->descriptor_bindings[ i ].type_description ) {
            ...
            throw exception::invalid_argument( std::string( "shader_module_reflection_t::get_member_pointer_maybe : Descriptor " ) + name + " has no type information.", __FILE__, __LINE__ );
          }
        }
        else {
          throw exception::invalid_argument( std::string( "shader_module_reflection_t::get_member_pointer_maybe : Descriptor " ) + name + " is not an uniform buffer or storage buffer.", __FILE__, __LINE__ );
        }
      }
      else if( strlen( reflect->descriptor_bindings[ i ].name ) == 0u ) {
        const auto descriptor_type = reflect->descriptor_bindings[ i ].descriptor_type;
