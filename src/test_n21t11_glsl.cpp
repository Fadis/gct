...
int main() {
  ...
  for( unsigned int i = 0u; i != 1000000; ++i ) {
    ...
    const auto encoded = n21t11_encode( normal, tangent );
    const auto decoded_normal = n21t11_decode_normal( encoded );
    const auto decoded_tangent = n21t11_decode_tangent( encoded, decoded_normal );
    if( !simeq( normal, decoded_normal, 0.017f ) || !simeq( tangent, decoded_tangent, 0.017f ) ) {
      std::cout <<
                   ( simeq( normal, decoded_normal, 0.017f ) ? "[  OK  ] " : "[FAILED] " ) <<
                   "normal ( " << normal.x << ", " << normal.y << ", " << normal.z << " ) " <<
                   "( " << decoded_normal.x << ", " << decoded_normal.y << ", " << decoded_normal.z << " )" <<
                   std::endl;
      std::cout <<
                   ( simeq( tangent, decoded_tangent, 0.017f ) ? "[  OK  ] " : "[FAILED] " ) <<
